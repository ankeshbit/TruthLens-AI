...
// Stable context map — survives re-renders
...
const LightTunnel = ({
  ...
  // ── Mount: create renderer & start RAF loop ──────────────────
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uOpacity:       { value: 1.0 },
        uCableColor:    { value: new Float32Array(hexToRgb('#4F7CFF')) },
        uPulseColor:    { value: new Float32Array(hexToRgb('#85A8FF')) },
        uTunnelColor:   { value: new Float32Array(hexToRgb('#4F7CFF')) },
  ...
  // ── Prop updates (no re-mount needed) ───────────────────────
  useEffect(() => {
    ...
    u.uSway.value          = sway;
    u.uSize.value          = size;
    (u.uCenter.value as Float32Array)[0] = centerX;
    (u.uCenter.value as Float32Array)[1] = centerY;
    u.uGlow.value          = glow;
    ...
    const setVec3 = (uniform: { value: Float32Array }, hex: string) => {
      const rgb = hexToRgb(hex);
      uniform.value[0] = rgb[0];
      uniform.value[1] = rgb[1];
      uniform.value[2] = rgb[2];
    };
    setVec3(u.uCableColor  as { value: Float32Array }, cableColor);
    setVec3(u.uPulseColor  as { value: Float32Array }, pulseColor);
    setVec3(u.uTunnelColor as { value: Float32Array }, tunnelColor);
  }, [
    cableColor, pulseColor, tunnelColor, tunnelOpacity,
    speed, flowDirection, pulseSpeed, pulseLength, pulseBlend, pulseWidth,
    cableCount, thickness, rimWidth, waviness, sway, size,
    centerX, centerY, glow, fadeNear, fadeFar, brightness,
    colorVariance, grain, grainIntensity, opacity, mouseInteraction, mouseStrength,
  ]);
